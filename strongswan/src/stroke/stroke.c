...
static int trigger_dpd(char *name, int retran_to, int retran_tries, int retran_base)
{
	stroke_msg_t msg;

	msg.type = STR_DPD;
	msg.length = offsetof(stroke_msg_t, buffer);
	msg.force_dpd.name = push_string(&msg, name);
	msg.force_dpd.retran_to = retran_to;
	msg.force_dpd.retran_tries = retran_tries;
	msg.force_dpd.retran_base = retran_base;
	return send_stroke_msg(&msg);
}
...
int main(int argc, char *argv[])
{
	...
	switch (token->kw)
	{
		...
		case STROKE_DPD:
			if (argc < 6)
			{
				exit_usage("\"dpd\" needs a connection name, timeout, retries and base");
			}
			res = trigger_dpd(argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
			break;
