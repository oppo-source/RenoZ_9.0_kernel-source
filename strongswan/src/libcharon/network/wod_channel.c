...
static char* wod_get_string_of_emergency_numbers(linked_list_t *list);
...
void notify_wod(wo_notify_cmd_t cmd, const char *conn_name, void* param)
{
	...
	{
		/*
		 * ATTACH notify format: ipsecattach=<sim_id>,<seq_num>,<status>,<sub_error>,<ipv6_address>,
		 *                                   <ipv4_address>,<backoff_time>,<mobike_support>
		 */
		case N_ATTACH:
			if(prop && prop->backoff_time > 0)
			{
				snprintf(tmp, MAX_BUF_LEN, "ipsecattach=%s,%d,%d,\"%s\",\"%s\",%d,%d",
						sim_sn_info, prop->status, prop->sub_error, prop->ip6[0], prop->ip[0],
						prop->backoff_time, prop->mobike_support);
			}
			else
			{
				if(prop)
				{
					snprintf(tmp, MAX_BUF_LEN, "ipsecattach=%s,%d,%d,\"%s\",\"%s\",0,%d",
							sim_sn_info, prop->status, prop->sub_error, prop->ip6[0], prop->ip[0],
							prop->mobike_support);
				}
			}
		...
		case N_OOS:
			...
			break;
		case N_EMERGENCY_NUMBERS:
			{
				linked_list_t *list = (linked_list_t*) param;
				char* em_nums_str = wod_get_string_of_emergency_numbers(list);
				snprintf(tmp, MAX_BUF_LEN, "ipsecemergnumbers=%s,%s", sim_sn_info, em_nums_str);
				free(em_nums_str);
			}
			break;
		case N_EMERGENCY_SUPPORT:
			{
				snprintf(tmp, MAX_BUF_LEN, "ipsecemergsupport=%s,%u", sim_sn_info, *((unsigned int*)param));
			}
			break;
...
static char* wod_get_string_of_emergency_numbers(linked_list_t *list)
{
	char tmp_data[MAX_BUF_LEN] = {0};
	int tmp_len = 0;
	em_call_number_t *current = NULL;
	enumerator_t *enumerator;

	snprintf(tmp_data, MAX_BUF_LEN, "%d", list->get_count(list));
	tmp_len = strlen(tmp_data);

	enumerator = list->create_enumerator(list);
	while (enumerator->enumerate(enumerator, &current))
	{
		snprintf(tmp_data + tmp_len, MAX_BUF_LEN, ",%d:%d", current->code, current->number);
		tmp_len = strlen(tmp_data);
	}
	enumerator->destroy(enumerator);
	return strdup(tmp_data);
}

void notify_wod_fast_reauth(void *value)
{
	char rxbuf[MAX_BUF_LEN] = {0};
	char tmp[MAX_BUF_LEN] = {0};
	int rxlen = 0;
	...
	snprintf(tmp, MAX_BUF_LEN, "ipsecfastreauth=%s", (char *)value);

	if (!is_ap_load_type("user"))
	{
		DBG2(DBG_IKE, "notify_wod_fast_reauth:[%s]", tmp);
	}
