								  ...
								  stroke_msg_t *msg, bool local, bool primary)
{
	...
	if (end->idr_apn)
	{
		...
		DBG1(DBG_CFG, "IDr_APN: %s", SENSITIVE_LOG(end->idr_apn));
	}
	return cfg;
}
								  ...
								  stroke_msg_t *msg, ike_cfg_t *ike_cfg)
{
	...
	peer_cfg->set_wod_options(peer_cfg,
		...
		msg->add_conn.pcscf_restore, msg->add_conn.device_identity,
		msg->add_conn.reauth_mbb, msg->add_conn.detach_timeout,
		msg->add_conn.redirect_enable, msg->add_conn.max_redirects,
		msg->add_conn.redirect_loop_detect, msg->add_conn.emergency_numbers,
		msg->add_conn.me.imei_format);
