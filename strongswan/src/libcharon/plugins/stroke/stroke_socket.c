...
static void stroke_add_conn(private_stroke_socket_t *this, stroke_msg_t *msg)
{
	...
	DBG2(DBG_CFG, "  enable redirect = %s", msg->add_conn.redirect_enable?"yes" : "no");
	DBG2(DBG_CFG, "  max redirects = %d", msg->add_conn.max_redirects);
	DBG2(DBG_CFG, "  redirect loop detect = %d", msg->add_conn.redirect_loop_detect);
	DBG2(DBG_CFG, "  emergency_numbers=%d", msg->add_conn.emergency_numbers);
...
{
	pop_string(msg, &msg->force_dpd.name);
