	...
	private_stroke_control_t *this, stroke_msg_t *msg, FILE *out)
{
	...
	DBG1(DBG_CFG, "data path (%s) is %s", msg->data_path_status_notify.if_name,
					data_path_ready ? "ready" : "not ready");
	hydra->kernel_interface->notify_data_path(hydra->kernel_interface,
						msg->data_path_status_notify.if_name, data_path_ready);
}

METHOD(stroke_control_t, wdrv_keepalive_notify, void,
	private_stroke_control_t *this, stroke_msg_t *msg, FILE *out)
{
	char *name;
	u_int32_t id;
	...
	uintptr_t ike_unique_id;

	if (!parse_specifier(msg->wdrv_keepalive_notify.name, &id, &name, &child, &all))
	{
		DBG1(DBG_CFG, "error parsing specifier string");
		return;
	}
	...
	enumerator = ike_list->create_enumerator(ike_list);
	while (enumerator->enumerate(enumerator, &ike_unique_id))
	{
		ike_sa = charon->ike_sa_manager->checkout_by_id(charon->ike_sa_manager,
														ike_unique_id, FALSE);
		if (ike_sa) {
			ike_sa->set_wdrv_keepalive(ike_sa, (bool)msg->wdrv_keepalive_notify.enable);
	...
	if (ike_list->get_count(ike_list) == 0)
	{
		DBG1(DBG_CFG, "no IKE_SA named '%s' found", name);
	}
	ike_list->destroy(ike_list);
}
...
stroke_control_t *stroke_control_create()
{
	...
	INIT(this,
		.public = {
			...
			.wdrv_keepalive_notify = _wdrv_keepalive_notify,
		},
