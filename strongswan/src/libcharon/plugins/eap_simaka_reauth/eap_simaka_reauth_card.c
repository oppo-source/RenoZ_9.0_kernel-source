...
static void notify_fast_reauth_id(identification_t *id, identification_t *next)
{
	char imsi[IMSI_LEN]  = {0};
	chunk_t data;
	char tmp[MAX_BUF_LEN] = {0};

	if (!id || !next || get_imsi_from_id(id, imsi)!= 0)
	{
		return;
	}

	data = next->get_encoding(next);
	if (data.ptr)
	{
		snprintf(tmp, sizeof(tmp), "%s,%s", imsi, data.ptr);
	}
	else
	{
		snprintf(tmp, sizeof(tmp), "%s,", imsi);
	}
	notify_wod_fast_reauth(tmp);
