	...
	bool up)
{
	...
	else
	{
		...
		unsigned int nw_emerg_sup = ike_sa->get_nw_emergency_support(ike_sa);
		if (nw_emerg_sup)
		{
			notify_wod(N_EMERGENCY_SUPPORT, config->get_name(config), &nw_emerg_sup);
		}
		this->prop.status = ike_sa->get_status_code(ike_sa);
		this->prop.sub_error = ike_sa->get_sub_error_code(ike_sa);
		...
		if(ike_sa->supports_extension(ike_sa, EXT_MOBIKE) == true)
		{
			this->prop.mobike_support = 1;
		} else {
			this->prop.mobike_support = 0;
		}
