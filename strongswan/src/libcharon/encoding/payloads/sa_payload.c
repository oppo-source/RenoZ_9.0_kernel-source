...
static bool match_proposals(proposal_substructure_t *substruct1, proposal_substructure_t *substruct2)
{
	proposal_t *proposal1, *proposal2;
	linked_list_t *proposal_list1, *proposal_list2;
	enumerator_t *e1, *e2;

	bool match = true;

	proposal_list1 = linked_list_create();
	proposal_list2 = linked_list_create();
	
	substruct1->get_proposals(substruct1, proposal_list1);
	substruct2->get_proposals(substruct2, proposal_list2);

	e1 =  proposal_list1->create_enumerator(proposal_list1);
	e2 =  proposal_list2->create_enumerator(proposal_list2);

	if (proposal_list1->get_count(proposal_list1) != proposal_list2->get_count(proposal_list2))
	{
		match = false;
		DBG2(DBG_IKE, "both proposals count is not same");
	}
	while (match && e1->enumerate(e1, &proposal1))
	{
		if (e2->enumerate(e2, &proposal2))
		{
			if (!proposal1->equals(proposal1, proposal2))
			{
				match = false;
				DBG2(DBG_IKE, "proposals content not matched");
			}
		}
	}
	...
	proposal_list1->destroy(proposal_list1);
	proposal_list2->destroy(proposal_list2);
	...
	private_sa_payload_t *this)
{
	...
	while (enumerator->enumerate(enumerator, &substruct))
	{
		...
		int current_id = substruct->get_protocol_id(substruct);

		/* check if a proposal has a single protocol */
		if (current_number == struct_number)
		{
			proposal_substructure_t *last_substruct;
			substructs->get_last(substructs, (void **)&last_substruct);
			DBG1(DBG_IKE, "Proposal number %d is same for both proposals", struct_number);
			if (current_id != struct_id || !match_proposals(substruct, last_substruct))
			{
