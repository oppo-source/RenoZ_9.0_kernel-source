...
struct private_ike_sa_t {
	...
	/**
	 * The heap in which the retransmission job during WiFi OOS are stored.
	 */
	array_t *oos_retransmit_job_queue;

	/**
	 * Time of last DPD triggered
	 */
	time_t last_dpd_trigger_time;

	/**
	 * Retransmission timeout/tries/base of Force DPD
	 */
	u_int32_t force_dpd_retran_to;
	u_int16_t force_dpd_retran_tries;
	u_int16_t force_dpd_retran_base;
	...
	/**
	 * network supports emergency service
	 */
	bool nw_emergency_support;
	...
	/**
	 *  Local host address changed in WIFI connecitvity
	 */
	bool local_address_changed;
	...
	*
	*/
	redirect_state_t ike_redirect_state;

	/**
	*redirect from ike_sa
	*
	*/
	ike_sa_t * redirect_previous_ike_sa;
};
...
static void resume_retransmit_job(private_ike_sa_t *this);
	...
	private_ike_sa_t *this)
{
	...
	// When the system time changed, the diff may be negative value because wrong value from kernel
	if (diff >= this->keepalive_interval || diff < 0)
	{
		packet_t *packet;
		chunk_t data;
...
#ifdef STRONGSWAN_UNIT_TEST
		lib->scheduler->ut_start_timer(lib->scheduler,job,
				(delay - diff)*1000, UT_SIMULATE_DPD_TIMER_PROPERTY);
#else
...
METHOD(ike_sa_t, send_force_dpd, status_t,
	private_ike_sa_t *this, u_int32_t retran_to,
	u_int16_t retran_tries, u_int16_t retran_base)
{
	if (this->state == IKE_PASSIVE)
	{
		notify_wod(N_FORCE_DPD, get_name(this), (void*)(intptr_t)FORCE_DPD_FAIL);
		return INVALID_STATE;
	}
	DBG1(DBG_IKE, "sending FORCE_DPD request");
	this->force_dpd_retran_to = retran_to;
	this->force_dpd_retran_tries = retran_tries;
	this->force_dpd_retran_base = retran_base;
	this->task_manager->queue_force_dpd(this->task_manager);
	if (this->task_manager->busy(this->task_manager))
	{
		DBG1(DBG_IKE, "an exchange is in the air, activate FORCE_DPD task and wait for exchange done");
		this->task_manager->activate_force_dpd(this->task_manager);
		return SUCCESS;
	}
	return this->task_manager->initiate(this->task_manager);
}

METHOD(ike_sa_t, get_force_dpd_retran_to, u_int32_t,
	private_ike_sa_t *this)
{
	return this->force_dpd_retran_to;
}

METHOD(ike_sa_t, get_force_dpd_retran_tries, u_int16_t,
	private_ike_sa_t *this)
{
	return this->force_dpd_retran_tries;
}

METHOD(ike_sa_t, get_force_dpd_retran_base, u_int16_t,
	private_ike_sa_t *this)
{
	return this->force_dpd_retran_base;
}
...
{
	/* Already configure "wdrv_keepalive", no need to care the "wdrv_keepalive" configuration */
	if (!this->read_keepalive_cfg)
	{
		this->keepalive_interval = this->peer_cfg->get_keepalive_time(this->peer_cfg);
		this->read_keepalive_cfg = TRUE;
...
METHOD(ike_sa_t, get_wdrv_keepalive, bool,
	private_ike_sa_t *this)
{
...
METHOD(ike_sa_t, notify_wod_dpd_interval, void,
	private_ike_sa_t *this)
{
	uint32_t interval;
	time_t now = time_monotonic(NULL);
	if (this->last_dpd_trigger_time)
	{
		interval = (uint32_t)(now - this->last_dpd_trigger_time);
	}
	else
	{
		interval = (uint32_t)(now - this->stats[STAT_ESTABLISHED]);
	}
	notify_wod(N_DPD_INTERVAL, get_name(this), (void*)(intptr_t)interval);
	this->last_dpd_trigger_time = now;
}

METHOD(ike_sa_t, get_redirect_previous_ike_sa, ike_sa_t*,
	private_ike_sa_t *this)
{
	return this->redirect_previous_ike_sa;
}

METHOD(ike_sa_t, set_redirect_previous_ike_sa, void,
	private_ike_sa_t *this, ike_sa_t* previous_ike_sa)
{
	this->redirect_previous_ike_sa = previous_ike_sa;
}
...
METHOD(ike_sa_t, get_local_address_changed, bool,
	private_ike_sa_t *this)
{
	return this->local_address_changed;
}

METHOD(ike_sa_t, set_local_address_changed, void,
	private_ike_sa_t *this, bool enable)
{
	this->local_address_changed = enable;
}

METHOD(ike_sa_t, get_redirect_state, redirect_state_t,
	private_ike_sa_t *this)
{
	return this->ike_redirect_state;
}

METHOD(ike_sa_t, set_redirect_state, void,
	private_ike_sa_t *this, redirect_state_t status_code)
{
	this->ike_redirect_state = status_code;
}
...
METHOD(ike_sa_t, set_nw_emergency_support, void,
	private_ike_sa_t *this, bool flag)
{
	this->nw_emergency_support = flag;
}

METHOD(ike_sa_t, get_nw_emergency_support, bool,
	private_ike_sa_t *this)
{
	return this->nw_emergency_support;
}
...
static bool redirect_established(private_ike_sa_t *this, identification_t *to)
{
	...
	/*
	*store the redirect from ike sa
	*/
	new->set_redirect_previous_ike_sa(new, (ike_sa_t*)this);
	...
	if (other)
	{
		...
		other_port = this->other_host->get_port(this->other_host);
#endif

		resolve_hosts(new_priv);
#ifdef STRONGSWAN_UNIT_TEST
		new_priv->other_host->set_port(new_priv->other_host, other_port);
#endif

		new_priv->redirected_at = array_create(sizeof(time_t), this->peer_cfg->get_max_redirects(this->peer_cfg));
...
static bool redirect_connecting(private_ike_sa_t *this, identification_t *to)
{
	...
	resolve_hosts(this);
#ifdef STRONGSWAN_UNIT_TEST
	this->other_host->set_port(this->other_host, other_port);
#endif
	return TRUE;
}
...
static bool redirect_count_exceeded(private_ike_sa_t *this)
{
	...
	if(!this->peer_cfg)
	{
		DBG1(DBG_IKE, "can not get peer_cfg");
		return FALSE;
	}
	...
	while (array_get(this->redirected_at, ARRAY_HEAD, &redirect) &&
		   now - redirect >= this->peer_cfg->get_redirect_loop_detect(this->peer_cfg))
	{
		array_remove(this->redirected_at, ARRAY_HEAD, NULL);
	}
	if (array_count(this->redirected_at) < this->peer_cfg->get_max_redirects(this->peer_cfg))
	{
		if (!this->redirected_at)
		{
			this->redirected_at = array_create(sizeof(time_t), this->peer_cfg->get_max_redirects(this->peer_cfg));
		}
	...
	private_ike_sa_t *this, identification_t *gateway)
{
	...
	if (this->peer_cfg && !this->peer_cfg->get_redirect_enable(this->peer_cfg))
	{
	...
	if (redirect_count_exceeded(this))
	{
		DBG1(DBG_IKE, "only %d redirects are allowed within %d seconds",
			 this->peer_cfg->get_max_redirects(this->peer_cfg), this->peer_cfg->get_redirect_loop_detect(this->peer_cfg));
		return FALSE;
	}
	...
	private_ike_sa_t *this, u_int32_t message_id)
{
	...
	if (this->task_manager->retransmit(this->task_manager, message_id) != SUCCESS)
	{
		...
		if (this->state != IKE_CONNECTING)
		{
			if (this->state == IKE_ESTABLISHED)
			{
				this->status_code = PDN_STATUS_LOCAL_RELEASE;
			}
			charon->bus->ike_updown(charon->bus, &this->public, FALSE);
		}
		return DESTROY_ME;
	...
	private_ike_sa_t *this, bool address)
{
...
#ifdef STRONGSWAN_UNIT_TEST
	if (is_current_path_valid(this) && ut_st_get_invoke_reauth() == 0)
	{
#else
	if (is_current_path_valid(this))
	{
		...
		/* Resume the deferred retransmit job because data path is ready */
		resume_retransmit_job(this);
	...
	this->local_address_changed = TRUE;
	this->task_manager->queue_dpd(this->task_manager);
	return this->task_manager->initiate(this->task_manager);
}
...
METHOD(ike_sa_t, defer_retransmit_job, void,
	private_ike_sa_t *this, job_t *job)
{
	array_insert_create(&this->oos_retransmit_job_queue, ARRAY_TAIL, job);

	DBG1(DBG_IKE, "Defer the retransmit job into IKE SA(total deferred: %d)",
			array_count(this->oos_retransmit_job_queue));
	return;
}
...
{
	job_t *job = NULL;
	while (array_remove(this->oos_retransmit_job_queue, ARRAY_HEAD, &job)) {
		lib->processor->queue_job(lib->processor, job);
		DBG1(DBG_IKE, "Resume the retransmit job from IKE SA");
	}
	...
	private_ike_sa_t *this)
{
	...
	array_destroy(this->oos_retransmit_job_queue);
	array_destroy_offset(this->peer_addresses, offsetof(host_t, destroy));
#ifdef ME
...

	{
		...
		*((u_int64_t*)spi_r.ptr) = this->ike_sa_id->get_responder_spi(this->ike_sa_id);
		...
		notify_wod_ikeesp(N_IKE_DECRYPT_DEL, get_name(this), (void *)tmp);
	}
						 ...
						 ike_version_t version)
{
	...
	INIT(this,
		.public = {
			...
			.get_wdrv_keepalive = _get_wdrv_keepalive,
			.set_keepalive_job_state = _set_keepalive_job_state,
			.get_oos = _get_oos,
			.set_oos = _set_oos,
			.get_oos_seq_num = _get_oos_seq_num,
			.notify_wod_dpd_interval = _notify_wod_dpd_interval,
			.get_redirect_previous_ike_sa = _get_redirect_previous_ike_sa,
			.set_redirect_previous_ike_sa = _set_redirect_previous_ike_sa,
			.get_status_code = _get_status_code,
			.get_redirect_state = _get_redirect_state,
			.set_redirect_state = _set_redirect_state,
			...
			.set_nw_emergency_support = _set_nw_emergency_support,
			.get_nw_emergency_support = _get_nw_emergency_support,
			...
			.get_local_address_changed = _get_local_address_changed,
			.set_local_address_changed = _set_local_address_changed,
			...
			.send_force_dpd = _send_force_dpd,
			.get_force_dpd_retran_to = _get_force_dpd_retran_to,
			.get_force_dpd_retran_tries = _get_force_dpd_retran_tries,
			.get_force_dpd_retran_base = _get_force_dpd_retran_base,
			...
			.defer_retransmit_job = _defer_retransmit_job,
		...
		.oos_seq_num = 0,
		.oos_start_time = 0,
		.last_dpd_trigger_time = 0,
		.force_dpd_retran_to = 0,
		.force_dpd_retran_tries = 0,
		.force_dpd_retran_base = 0,
		.status_code = 0,
		.sub_error_code = 0,
		.local_address_changed = FALSE,
		.backoff_timer_value = 0,
		.pcscf_need_update = FALSE,
		.notify_device_identity = FALSE,
		.ike_redirect_state = REDIRECT_NONE,
		.redirect_previous_ike_sa = NULL,
	);
