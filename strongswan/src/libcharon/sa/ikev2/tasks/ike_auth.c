	...
	private_ike_auth_t *this, message_t *message)
{
	...
	{	/* in the first IKE_AUTH ... */
		...
		/* indicate support for emergency call numbers feature */
		if (this->peer_cfg->get_emergency_numbers(this->peer_cfg))
		{
			message->add_notify(message, FALSE, EMERGENCY_CALL_NUMBERS,
								chunk_empty);
			DBG1(DBG_CFG, "add_notify : EMERGENCY_CALL_NUMBERS: %u", EMERGENCY_CALL_NUMBERS);
		}
	}
	...
	if (this->my_auth == NULL)
	{
			...
			this->peer_cfg->get_unique_policy(this->peer_cfg) != UNIQUE_NEVER)
		{
			...
			DBG1(DBG_CFG, "====== idi: %Y, idr: %Y, host: %Y ========",
					SENSITIVE_LOG_ID(idi), SENSITIVE_LOG_ID(idr), SENSITIVE_LOG_ID(id_host));
	...
	/* if need notify device_identity, add device_identity notify payload for IKE_AUTH request*/
	if (this->ike_sa->get_notify_device_identity(this->ike_sa))
	{
		char *imei_bcd = this->peer_cfg->get_imei_bcd_encoded(this->peer_cfg);
		if (imei_bcd)
		{
			u_int16_t size = this->peer_cfg->get_imei_bcd_encoded_size(this->peer_cfg);
	...
	private_ike_auth_t *this, message_t *message)
{
	...
	if (!this->do_another_auth && !this->expect_another_auth)
	{
		...
		DBG0(DBG_IKE, "IKE_SA %s[%d] established between %H[%Y]...%H[%Y]",
			 ...
			 SENSITIVE_LOG_HOST(this->ike_sa->get_my_host(this->ike_sa)),
			 SENSITIVE_LOG_ID(this->ike_sa->get_my_id(this->ike_sa)),
			 SENSITIVE_LOG_HOST(this->ike_sa->get_other_host(this->ike_sa)),
			 SENSITIVE_LOG_ID(this->ike_sa->get_other_id(this->ike_sa)));
...
static linked_list_t *emergency_call_numbers_data_parse(chunk_t data)
{
	int number;
	int number_bytes;
	linked_list_t *list;
	em_call_number_t *node;

	u_char *p_data = data.ptr;
	int total_bytes = data.len;
	
	list = linked_list_create();

	/* Parsing as per 10.5.3.13 of 3GPP TS 24.008 [46] starting from 3rd octet*/
	p_data += 2;
	total_bytes -= 2;
	...
	while (total_bytes > 0)
	{
		...
		int pos = 1;
		number = 0;
		node = (em_call_number_t*) malloc(sizeof(em_call_number_t));
		memset(node, 0, sizeof(em_call_number_t));
		number_bytes = (int) (p_data[0] - 1);
		node->code = (int) (p_data[1] & 0x1F);
		p_data += 2;
		total_bytes -= 2;
		for (int j = 0; j < number_bytes; ++j)
		{
			digit = (int) (p_data[j] & 0x0F);
			number += (digit * pos);
			pos *= 10;
			/* skip for odd digit number condition : last digit*/
			if (!((j == number_bytes - 1) && ((p_data[j] & 0xF0) == 0xF0)))
			{
				digit = (int) ((p_data[j] & 0xF0) >> 4);
				number += (digit * pos);
				pos *= 10;
			}
			else
			{
				DBG3(DBG_IKE, "[Parsing] Odd digit number, skipped last nibble");
			}
		}
		node->number = number;
		p_data += number_bytes;
		total_bytes -= number_bytes;
		list->insert_last(list, node);
		DBG3(DBG_IKE, "[Parsing]code:%d, number: %d", node->code, node->number);
	}
	return list;
}
		...
		if (payload->get_type(payload) == NOTIFY)
		{
			...
			switch (type)
			{
				...
				case EMERGENCY_SUPPORT:
					DBG1(DBG_IKE, "network supports emergency services, enabling nw_emergency status");
					this->ike_sa->set_nw_emergency_support(this->ike_sa, TRUE);
					break;
				case EMERGENCY_CALL_NUMBERS:
					if (this->peer_cfg->get_emergency_numbers(this->peer_cfg))
					{
						chunk_t data;
						linked_list_t *list;
						...
						if (data.len > 0 && data.len <= 48)
						{
							DBG1(DBG_IKE, "emergency call numbers received");
							list = emergency_call_numbers_data_parse(data);

							/* Send this list to WOD*/
							notify_wod(N_EMERGENCY_NUMBERS, this->ike_sa->get_name(this->ike_sa), (void*)list);
							list->destroy_function(list, free);
						}
						else
						{
							DBG1(DBG_IKE, "emergency call numbers received, but droped as not in valid rang");
						}
					}
					break;
				case PDN_CONNECTION_REJECTION:
				case MAX_CONNECTION_REACHED:
				case NON_3GPP_ACCESS_TO_EPC_NOT_ALLOWED:
				case USER_UNKNOWN:
				case NO_APN_SUBSCRIPTION:
				case AUTHORIZATION_REJECTED:
				case ILLEGAL_ME:
				case NETWORK_FAILURE:
				case RAT_TYPE_NOT_ALLOWED:
				case IMEI_NOT_ACCEPTED:
				case PLMN_NOT_ALLOWED:
				case UNAUTHENTICATED_EMERGENCY_NOT_SUPPORTED:
				default:
				{
					if (type <= 16383)
					{
...
//						return FAILED;
	...
	if (this->my_auth)
	{
		switch (this->my_auth->process(this->my_auth, message))
		{
			...
			case NEED_MORE:
				break;
			case VERIFY_ERROR:
				DBG1(DBG_IKE, "Receive EAP_FAILURE without error notify payload");
				if (this->ike_sa->get_status_code(this->ike_sa) == PDN_STATUS_OK)
				{
					this->ike_sa->set_status_code(this->ike_sa, PDN_STATUS_UNABLE_MAKE_IPSEC_TUNNEL);
					this->ike_sa->set_sub_error_code(this->ike_sa, AUTHENTICATION_FAILED);
				}
				goto peer_auth_failed;
			default:
				charon->bus->alert(charon->bus, ALERT_LOCAL_AUTH_FAILED);
	...
	if (!this->expect_another_auth && !this->do_another_auth && !this->my_auth)
	{
		...
		if (this->redirect_to)
		{
			this->ike_sa->set_redirect_state(this->ike_sa, REDIRECT_IKE_SA_ESTABLISHED_START);
