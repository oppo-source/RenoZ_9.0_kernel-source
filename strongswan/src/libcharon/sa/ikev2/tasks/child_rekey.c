	...
	private_child_rekey_t *this, message_t *message)
{
	...
	if (!this->child_create)
	{
		this->child_create = child_create_create(this->ike_sa,
									config->get_ref(config), TRUE, NULL, NULL, this->child_sa->get_rekey_tsi(this->child_sa), this->child_sa->get_rekey_tsr(this->child_sa));
	}
								  ...
								  u_int32_t spi)
{
	...
	else
	{
		...
		this->child_create = child_create_create(ike_sa, NULL, TRUE, NULL, NULL, NULL, NULL);
	}
