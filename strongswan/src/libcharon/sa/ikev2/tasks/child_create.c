...
bool check_previous_redirect_state(ike_sa_t *ike_sa, redirect_state_t redirect_state)
{
	bool ret = FALSE;
	ike_sa_t * redirect_from_ike_sa = ike_sa->get_redirect_previous_ike_sa(ike_sa);
	while(redirect_from_ike_sa)
	{
		if(redirect_from_ike_sa->get_redirect_state(redirect_from_ike_sa) == redirect_state)
		{
			ret = TRUE;
			DBG0(DBG_IKE,"found redirect_state ike_sa:%d",redirect_state);
			break;
		}
		redirect_from_ike_sa = redirect_from_ike_sa->get_redirect_previous_ike_sa(redirect_from_ike_sa);
	}
	return ret;
}

void set_previous_redirect_state(ike_sa_t *ike_sa, redirect_state_t redirect_state)
	{
		ike_sa_t * redirect_from_ike_sa = ike_sa->get_redirect_previous_ike_sa(ike_sa);
		while(redirect_from_ike_sa)
		{
			redirect_from_ike_sa->set_redirect_state(redirect_from_ike_sa, redirect_state);
			redirect_from_ike_sa = redirect_from_ike_sa->get_redirect_previous_ike_sa(redirect_from_ike_sa);
		}
	}
	...
	private_child_create_t *this, message_t *message)
{
	...
	{
		/*
		* set previous redirect state
		* if the redirect previous ike sa has establish child sa, can not notify again
		*/
		if (!this->rekey)
		{	/* invoke the child_up() hook if we are not rekeying */
			if(!check_previous_redirect_state(this->ike_sa, REDIRECT_CHILD_SA_ESTABLISHED_START))
			{
				set_previous_redirect_state(this->ike_sa, REDIRECT_SUCCESS);
			...
			{
				set_previous_redirect_state(this->ike_sa, REDIRECT_SUCCESS);
				DBG0(DBG_IKE,"redirect previous ike sa has establish child sa, so no need again");
			}
		}
	}
	else
