...
#define DBG_WITH_ANDROID_PROPERTY(prop_type, group, fmt, ...) \
   ...
   else if(get_cust_setting_bool(prop_type)){  DBG1(group, fmt, ##__VA_ARGS__); }

#define SENSITIVE_LOG(data) (is_ap_load_type("user") ? "***" : data)
#define SENSITIVE_LOG_ID(data) (is_ap_load_type("user") ? NULL : data)
#define SENSITIVE_LOG_HOST(data) (is_ap_load_type("user") ? NULL : data)
...
bool is_ap_load_type(const char *type);
#endif /** DEBUG_H_ @}*/
